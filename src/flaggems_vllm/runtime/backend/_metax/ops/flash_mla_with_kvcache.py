...
Includes MetaX C550-specific launch and scheduling paths.
"""
...
def _metax_choose_global_split_v1(
    ...
    base_programs = batch_size * seq_q * (num_heads_q // bh)

    max_kv = max(lengths)

    mean_kv = sum(lengths) / len(lengths)
...
def _metax_make_auto_split_cache_key(
    ...
    return (
        ...
        _metax_cache_seqlens_version(cache_seqlens),
    )
...
def _metax_auto_global_split_v2(
    ...
    if old_key == key and cached_split is not None:
    ...
    if cache_seqlens.device.type == "cpu":
        lengths = [int(x) for x in cache_seqlens.tolist()]
    else:
        lengths = [
            int(x)
            for x in (
                cache_seqlens.detach()
                .to(
                    device="cpu",
                    dtype=torch.int32,
                )
...
#
# Reuse the generic definition so callers that obtain metadata from
# flaggems_vllm.ops.flash_mla_with_kvcache can pass it directly to the
# MetaX backend implementation without Python type-identity mismatch.
# ============================================================================
...
def _sparse_decode_model1_kernel(
    ...
    for ck in range(NK):
        ...
        valid_ids = t_msk & (kv_ids >= 0) & (kv_ids < NUM_BLOCKS * PAGE_SIZE)
    ...
    if HAVE_EXTRA:
        ...
        for ck in range(ENK):
            ...
            valid_ids = (
                t_msk & (kv_ids >= 0) & (kv_ids < EXTRA_NUM_BLOCKS * EXTRA_PAGE_SIZE)
            )
            block_ids = tl.where(valid_ids, block_ids, 0)
            rel_ids = tl.where(valid_ids, rel_ids, 0)
...
def _dense_decode_kernel(
    ...
    for i in range(0, loop_time):
        ...
        # by stride_kv_bs.
        kv_page_number = tl.load(Block_table + offs_n // PAGE_SIZE).to(tl.int64)
        kv_loc = kv_page_number * PAGE_SIZE + (offs_n % PAGE_SIZE).to(tl.int64)
...
def flash_mla_with_kvcache(
    ...
    if num_splits is None:
        ...
        if _metax_auto_split_supported:
            ...
            num_splits = _metax_auto_global_split_v2(
                sched_meta,
                q,
                k_cache,
                cache_seqlens,
...
def _sparse_decode_dispatch(
    ...
    grid = lambda META: (batch_size * seq_q * triton.cdiv(num_heads_q, META["BH"]),)
...
def _dense_decode_dispatch(
    ...
    if num_splits is not None and num_splits > 1:
        try:
            is_metax = "metax" in torch.cuda.get_device_name(q.device).lower()
        except Exception:
            is_metax = False
...
#
# The caller may provide num_splits explicitly, or the MetaX SQ=1 policy may
...
def _metax_dense_decode_split_k_partial(
    ...
    q_base = q + i_b * stride_qb + i_sq * stride_qsq + h_base * stride_qh

    q_l = tl.load(
        q_base + offs_h[:, None] * stride_qh + offs_d[None, :],
        mask=mask_h[:, None],
        other=0.0,
    )

    q_r = tl.load(
        q_base + offs_h[:, None] * stride_qh + (DPH + offs_d)[None, :],
        mask=mask_h[:, None],
        other=0.0,
    )
    ...
    if HAVE_TAIL:
        ...
        q_tail = tl.load(
            q_base + offs_h[:, None] * stride_qh + (D + offs_td)[None, :],
    ...
    cache_seqlen = tl.load(cache_seqlens + i_b)
    ...
    page_begin = split_id * pages_per_split
    ...
    bt_row = block_table + i_b * stride_bt

    for logical_page in tl.range(
        page_begin,
        page_end,
    ):
        phys_page = tl.load(bt_row + logical_page)

        page_token_base = phys_page.to(tl.int64) * PAGE_SIZE
        ...
        for in_page_start in tl.range(
            ...
            logical_start = logical_page * PAGE_SIZE + in_page_start

            logical_tokens = logical_start + offs_n

            valid = logical_tokens < cache_seqlen

            kv_rows = page_token_base + in_page_start + offs_n
            ...
            k_l = tl.load(
                kv_cache + kv_rows[:, None] * stride_kvn + offs_d[None, :],
                mask=valid[:, None],
                other=0.0,
            )
            ...
            k_r = tl.load(
                kv_cache + kv_rows[:, None] * stride_kvn + (DPH + offs_d)[None, :],
                mask=valid[:, None],
                other=0.0,
            )
            ...
            if HAVE_TAIL:
                k_tail = tl.load(
                    kv_cache + kv_rows[:, None] * stride_kvn + (D + offs_td)[None, :],
                    mask=(valid[:, None] & (offs_td < TD)[None, :]),
                    other=0.0,
                )
            ...
            p_bf16 = p.to(q.dtype.element_ty)
    ...
    workspace_row = (split_id * BS + i_bsq) * G + h_base + offs_h

    po_base = partial_out + workspace_row[:, None] * D
...
def _metax_dense_decode_split_k_merge(
    ...
    for split_id in tl.static_range(
        ...
        row = (split_id * BS + i_bsq) * G + h_base + offs_h
    ...
    for split_id in tl.static_range(
        ...
        valid_part = part_lse != float("-inf")

        weight = tl.where(
            valid_part,
            tl.exp(part_lse - max_lse),
            0.0,
        )

        po_base = partial_out + row[:, None] * D
        ...
        acc_l += part_l * weight[:, None]

        acc_r += part_r * weight[:, None]

    valid_row = (denom > 0.0) & mask_h
    ...
    out_base = (
        ...
        + i_b * stride_ob
        + i_sq * stride_osq
        + (h_base + offs_h)[:, None] * stride_oh
    )

    tl.store(
        out_base + offs_d[None, :],
        (acc_l * inv_denom[:, None]).to(output.dtype.element_ty),
        mask=mask_h[:, None],
    )

    tl.store(
        out_base + DPH + offs_d[None, :],
        (acc_r * inv_denom[:, None]).to(output.dtype.element_ty),
        mask=mask_h[:, None],
    )
    ...
    tl.store(
        lse + i_b * stride_lseb + (h_base + offs_h) * stride_lseh + i_sq,
        final_lse,
        mask=mask_h,
    )
...
def _metax_dense_decode_split_k_launch(
    ...
    bsq = batch_size * seq_q

    # --------------------------------------------------------
    # Workspace
    #
    # Reuse the cached FP32 partial workspace for the selected geometry.
    # --------------------------------------------------------
    ...
    _metax_dense_decode_split_k_partial[partial_grid](
        q,
        q.stride(0),
        q.stride(1),
        q.stride(2),
    ...
    _metax_dense_decode_split_k_merge[merge_grid](
        partial_out,
        partial_lse,
        out,
        out.stride(0),
...
if HAS_TLE:
    ...
    def _tle_dense_decode_serial_fwd(
        ...
        q_l_ptr = q_base + offs_h[:, None] * DQK + offs_d[None, :]

        q_r_ptr = q_base + offs_h[:, None] * DQK + (DPH + offs_d)[None, :]
        ...
        if HAVE_TAIL:
            ...
            q_tail_ptr = q_base + offs_h[:, None] * DQK + (D + offs_td)[None, :]
        ...
        for logical_page in tl.range(num_pages):
            ...
            phys_page = tl.load(block_table_ptr + logical_page)
            ...
            for in_page_start in tl.range(
                ...
                k_l_ptr = kv_cache + kv_rows[:, None] * stride_kvn + offs_d[None, :]
        ...
        out_l_ptr = out_base + offs_h[:, None] * D + offs_d[None, :]

        out_r_ptr = out_base + offs_h[:, None] * D + (DPH + offs_d)[None, :]
